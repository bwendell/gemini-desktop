...
describe('ThemeContext Coverage', () => {
    ...
    it('handles initialization error by falling back to system preference', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to initialize theme'), expect.any(Error));
    ...
    it('handles setTheme error gracefully', async () => {
        window.electronAPI = {
            ...
            setTheme: vi.fn().mockImplementation(() => { throw new Error('Set failed'); }),
            onThemeChanged: vi.fn().mockReturnValue(() => { }),
        } as any;
