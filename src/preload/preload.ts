...
type IpcChannels = typeof import('../shared/constants/ipc-channels').IPC_CHANNELS;

declare const require: NodeRequire | undefined;

const loadIpcChannels = (): IpcChannels | null => {
    try {
        if (typeof require === 'function') {
            const module = require('../shared/constants/ipc-channels') as { IPC_CHANNELS?: IpcChannels };
            if (module?.IPC_CHANNELS) {
                return module.IPC_CHANNELS as IpcChannels;
            }
        }
    } catch (error) {}
    return null;
