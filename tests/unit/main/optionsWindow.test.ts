...
describe('OptionsWindow', () => {
    ...
    describe('create with tab', () => {
        it('passes settings tab hash in dev mode', () => {
            ...
            expect(win.loadURL).toHaveBeenCalledWith('http://localhost:1420/src/renderer/windows/options/options.html#settings');
        });
        ...
        it('navigates existing window to new tab', () => {
            const win1 = optionsWindow.create();
            win1.webContents.getURL = vi.fn().mockReturnValue('http://localhost:1420/src/renderer/windows/options/options.html');

            const win2 = optionsWindow.create('settings');

            expect(win1).toBe(win2);
            expect(win1.loadURL).toHaveBeenCalledWith('http://localhost:1420/src/renderer/windows/options/options.html#settings');
        });
    });
