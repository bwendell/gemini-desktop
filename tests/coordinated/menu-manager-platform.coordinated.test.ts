...
describe('MenuManager Platform Integration', () => {
    ...
    describe('on darwin (macOS)', () => {
        ...
        it('should include About and Settings in app menu on macOS', () => {
            ...
            const hasAbout = appMenu.submenu.some((item: any) =>
                item.label?.includes('About') || item.id === 'menu-app-about'
            );
            const hasSettings = appMenu.submenu.some((item: any) =>
                item.label?.includes('Settings') || item.id === 'menu-app-settings'
            );
        ...
        it('should include View menu with Always On Top on macOS', () => {
            ...
            const hasAlwaysOnTop = viewMenu.submenu.some((item: any) =>
                item.label?.includes('Always On Top') || item.id === 'menu-view-always-on-top'
            );
        ...
        it('should include Exit in File menu on Windows', () => {
            ...
            if (fileMenu?.submenu) {
                const hasExit = fileMenu.submenu.some((item: any) =>
                    item.label?.includes('Exit') || item.role === 'quit'
                );
    ...
    describe('on linux', () => {
        ...
        it('should include Quit in File menu on Linux', () => {
            ...
            if (fileMenu?.submenu) {
                const hasQuit = fileMenu.submenu.some((item: any) =>
                    item.label?.includes('Quit') ||
                    item.label?.includes('Exit') ||
                    item.role === 'quit'
                );
        ...
        describe.each(['darwin', 'win32', 'linux'] as const)('on %s', (platform) => {
            ...
            it('should trigger WindowManager methods via menu callbacks', () => {
                ...
                if (viewMenu?.submenu) {
                    ...
                    const alwaysOnTopItem = viewMenu.submenu.find((item: any) =>
                        item.label?.includes('Always on Top') ||
                        item.id === 'always-on-top'
                    );
