    ...
    describe.each(['darwin', 'win32', 'linux'] as const)('on %s', (platform) => {
                ...
                if (platform === 'darwin') {
                    ...
                    expect(buildCall).toEqual(
                        expect.arrayContaining([
                            expect.objectContaining({ label: 'Gemini Desktop' })
                        ])
                    );
                } else {
        ...
        describe('Context Menu', () => {
            it('should setup context menu for webContents', () => {
                ...
                expect((mainWindow.webContents.on as any)).toHaveBeenCalledWith(
                    'context-menu',
                    expect.any(Function)
                );
            });
        ...
        describe('Menu Actions', () => {
            it('should open options window from menu', () => {
                ...
                const fileOrAppMenu = template.find((menu: any) =>
                    menu.label === 'File' || menu.role === 'appMenu'
                );
                expect(fileOrAppMenu).toBeDefined();

                // Find Settings submenu item
                const settingsItem = fileOrAppMenu.submenu?.find?.(
            ...
            it('should toggle full screen from View menu', () => {
                ...
                const fullScreenItem = viewMenu.submenu?.find(
                    (item: any) => item.role === 'togglefullscreen'
                );
