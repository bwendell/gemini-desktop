/**
 * Shared Wait Utilities for Deterministic Test Waits.
 *
 ...
 *
 * Extracted from tests/e2e/helpers/waitUtilities.ts for reuse across
 * e2e and integration test suites. Behavioral parity is maintained exactly.
 *
 * @module wait-utilities
...
import { E2E_TIMING } from './timing-constants';
...
/**
 * Minimal WdioElement type for shared context.
 * Uses WebdriverIO.Element which is available via @wdio/globals types.
 * The e2e helper wdio-electron.d.ts extends this with additional methods,
 * but the shared wait utilities only require isExisting() and getCSSProperty().
 */
...
export async function waitForUIState(
    ...
    TestLogger.info('waitUtilities', `${logPrefix}Waiting for UI state (timeout: ${timeout}ms)`);

    while (Date.now() - startTime < timeout) {
        try {
            if (await condition()) {
                const elapsed = Date.now() - startTime;
                TestLogger.info('waitUtilities', `${logPrefix}✓ Condition met after ${elapsed}ms`);
                return true;
            }
        } catch (error) {
            // Condition threw error, treat as not met and continue polling
            TestLogger.info('waitUtilities', `${logPrefix}Condition check error: ${error}`);
        }

        await browser.pause(interval);
    }

    TestLogger.info('waitUtilities', `${logPrefix}✗ Timeout waiting for UI state after ${timeout}ms`);
    return false;
}
...
export async function waitForIPCRoundTrip(
    ...
    TestLogger.info('waitUtilities', `Executing IPC action (timeout: ${timeout}ms)`);
    ...
    while (Date.now() - startTime < timeout) {
        try {
            if (await verification()) {
                const elapsed = Date.now() - startTime;
                TestLogger.info('waitUtilities', `✓ IPC round-trip complete after ${elapsed}ms`);
                return;
            }
        } catch {
    ...
    TestLogger.info('waitUtilities', `✗ IPC verification timeout after ${timeout}ms`);
...
export async function waitForWindowTransition(
    ...
    TestLogger.info('waitUtilities', `[${description}] Waiting for window transition (timeout: ${timeout}ms)`);
        ...
        try {
            ...
            if (conditionResult) {
                ...
                if (conditionMetTime === null) {
                    ...
                    TestLogger.info('waitUtilities', `[${description}] Condition met, checking stability...`);
                ...
                if (conditionMetTime !== null) {
                    TestLogger.info('waitUtilities', `[${description}] Condition unstable, resetting stability check`);
    ...
    TestLogger.info('waitUtilities', `[${description}] ✗ Window transition timeout after ${timeout}ms`);
    return false;
}
...
export async function waitForAnimationSettle(
    ...
    TestLogger.info('waitUtilities', `✗ Animation settle timeout after ${timeout}ms`);
    return false;
}
...
export async function waitForFullscreenTransition(
        ...
        try {
            ...
            if (currentState === targetState) {
                ...
                if (stableStartTime === null) {
                    stableStartTime = now;
                    TestLogger.info('waitUtilities', `[${stateName}] State matched, checking stability...`);
                ...
                if (stableStartTime !== null) {
                    TestLogger.info('waitUtilities', `[${stateName}] State unstable, resetting stability check`);
    ...
    TestLogger.info('waitUtilities', `✗ Fullscreen transition timeout after ${timeout}ms`);
    return false;
}
...
export async function waitForMacOSWindowStabilize(
    ...
    if (!isMacOS) {
        TestLogger.info('waitUtilities', `[${description}] Skipping macOS stabilization (not on macOS)`);
        return true;
    }
    ...
    if (!condition) {
        ...
        TestLogger.info('waitUtilities', `[${description}] Waiting ${stabilizationDelay}ms for macOS stabilization`);
...
export async function waitForWindowCount(expectedCount: number, timeout = 5000): Promise<boolean> {
    ...
    TestLogger.info('waitUtilities', `Waiting for window count to be ${expectedCount} (timeout: ${timeout}ms)`);
    ...
    while (Date.now() - startTime < timeout) {
        ...
        if (handles.length === expectedCount) {
            const elapsed = Date.now() - startTime;
            TestLogger.info('waitUtilities', `✓ Window count is ${expectedCount} after ${elapsed}ms`);
            return true;
        }
...
export async function waitForDuration(durationMs: number, description?: string): Promise<void> {
    const logPrefix = description ? `[${description}] ` : '';
    TestLogger.info('waitUtilities', `${logPrefix}Waiting ${durationMs}ms (intentional duration wait)`);

    await browser.pause(durationMs);

    TestLogger.info('waitUtilities', `${logPrefix}✓ Duration wait complete`);
}
