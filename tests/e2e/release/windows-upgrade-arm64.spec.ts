...
describe('Windows upgrade arm64', () => {
    beforeEach(async () => {
    ...
    it('runs the promoted arm64 build after upgrading from a baseline installer', async () => {
        if (process.arch !== 'arm64') {
            throw new Error(`Expected arm64 runtime for upgrade validation, received ${process.arch}`);
        }

        const required = ['BASELINE_INSTALLER_PATH', 'BASELINE_VERSION', 'TARGET_VERSION'];
        for (const key of required) {
            if (!process.env[key]) {
                throw new Error(`${key} is required for the Windows arm64 upgrade spec.`);
            }
        }
        ...
        expect(runtimeInfo.arch).toBe('arm64');
