...
describe('Quick Chat Text Injection', () => {
  ...
  describe('Text Injection Content Handling (No Submit)', () => {
    ...
    it('should inject very long text (1000+ characters)', async () => {
      ...
      E2ELogger.info(
        'injection-content',
        `Long text (${longText.length} chars) injected (NOT submitted)`
      );
    });
  ...
  describe('Rapid Injection Handling (No Submit)', () => {
    ...
    it('should handle multiple rapid injections without errors', async () => {
      const texts = ['First quick message', 'Second quick message', 'Third quick message'];
      ...
      results.forEach((result, index) => {
        ...
        E2ELogger.info(
          'injection-rapid',
          `Injection ${index + 1}: iframeFound=${result.iframeFound}`
        );
      });
  ...
  describe('Integration with Main Window (No Submit)', () => {
    it('should complete workflow: show -> inject -> verify (NO submit)', async () => {
      ...
      E2ELogger.info(
        'injection-integration',
        'Injection workflow completed (NO message sent to Gemini)'
      );
    });
    ...
    it('should handle injection failure gracefully (e.g., when Gemini DOM changes)', async () => {
      E2ELogger.info(
        'injection-error',
        'Testing injection failure handling with invalid selectors'
      );
      ...
      E2ELogger.info(
        'injection-error',
        `Invalid injection result: ${JSON.stringify(invalidResult)}`
      );
