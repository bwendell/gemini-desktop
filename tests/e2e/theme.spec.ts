...
describe('Theme', () => {
    describe('Theme Switching', () => {
        ...
        it('should apply correct text colors to Options titlebar in light and dark modes', async () => {
            ...
            const titleElement = await wdioBrowser.$('[data-testid="options-titlebar-title"]');
    ...
    describe('Selector Visual', () => {
        ...
        it('should display three theme cards with visual previews', async () => {
            ...
            const previews = await wdioBrowser.$$('.theme-card__preview');
            expect(previews.length).toBe(3);

            const labels = await wdioBrowser.$$('.theme-card__label');
            expect(labels.length).toBe(3);

            const systemCard = await wdioBrowser.$(optionsPage.themeCardSelector('system'));
            const lightCard = await wdioBrowser.$(optionsPage.themeCardSelector('light'));
            const darkCard = await wdioBrowser.$(optionsPage.themeCardSelector('dark'));
        ...
        it('should show checkmark indicator on currently selected theme', async () => {
            ...
            await wdioBrowser.waitUntil(
                async () => !(await wdioBrowser.$('[data-testid="theme-checkmark-system"]')).isExisting(),
                {
                    timeout: 500,
            ...
            await wdioBrowser.waitUntil(
                async () => !(await wdioBrowser.$('[data-testid="theme-checkmark-dark"]')).isExisting(),
                {
                    timeout: 500,
            ...
            await wdioBrowser.waitUntil(
                async () => !(await wdioBrowser.$('[data-testid="theme-checkmark-light"]')).isExisting(),
                {
                    timeout: 500,
    ...
    describe('Selector Keyboard', () => {
        ...
        it('should have proper radiogroup and radio ARIA roles', async () => {
            await withOptionsWindowViaMenu(async () => {
                const themeSelector = await wdioBrowser.$(optionsPage.themeSelectorSelector);
        ...
        it('should select theme with Enter key when card is focused', async () => {
            await withOptionsWindowViaMenu(async () => {
                ...
                const darkCheckmark = await wdioBrowser.$('[data-testid="theme-checkmark-dark"]');
