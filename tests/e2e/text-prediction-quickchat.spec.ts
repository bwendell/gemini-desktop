      ...
      try {
        ...
        if (isModelReady) {
          ...
          try {
            ...
            E2ELogger.info(
              'text-prediction-quickchat',
              'Ghost text did not appear (model may be slow)'
            );
            ...
            E2ELogger.info(
              'text-prediction-quickchat',
              `Ghost text displayed: ${isGhostDisplayed}`
            );
          }
        } else {
          ...
          E2ELogger.info(
            'text-prediction-quickchat',
            'Model not ready, skipping ghost text verification'
          );
    ...
    it('should accept prediction text when Tab is pressed in Quick Chat', async () => {
      ...
      try {
        ...
        if (isModelReady) {
          ...
          E2ELogger.info(
            'text-prediction-quickchat',
            `Input after Tab (no prediction): "${inputValue}"`
          );
        }
      ...
      try {
        ...
        if (isModelReady) {
          ...
          try {
            ...
            E2ELogger.info(
              'text-prediction-quickchat',
              '✓ Ghost text dismissed by continued typing'
            );
          ...
          E2ELogger.info(
            'text-prediction-quickchat',
            'Model not ready, skipping ghost text dismissal test'
          );
        }
  ...
  describe('Enter Key Submission (10.10)', () => {
    it('should submit only original text when Enter is pressed, ignoring any pending prediction', async () => {
      ...
      try {
        ...
        E2ELogger.info(
          'text-prediction-quickchat',
          '✓ Enter submission completed with original text only'
        );
      } finally {
    ...
    it('should submit original text even when prediction ghost text is visible', async () => {
      ...
      try {
        ...
        E2ELogger.info(
          'text-prediction-quickchat',
          '✓ Submit via Enter ignored prediction successfully'
        );
      } finally {
        try {
          await quickChatPage.hide();
