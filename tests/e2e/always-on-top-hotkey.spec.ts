...
describe('Always On Top - Hotkey Toggle', () => {
    ...
    describe('Basic Hotkey Functionality', () => {
        it('should toggle always-on-top when hotkey is pressed', async () => {
            ...
            E2ELogger.info('always-on-top-hotkey', `Initial state: ${wasEnabled ? 'enabled' : 'disabled'}`);
            ...
            E2ELogger.info('always-on-top-hotkey', `State after hotkey: ${newState?.enabled ? 'enabled' : 'disabled'}`);
        ...
        it('should toggle state when hotkey is pressed multiple times', async () => {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                E2ELogger.info('always-on-top-hotkey', `Toggle ${i + 1}: ${currentState?.enabled ? 'enabled' : 'disabled'}`);
            }
    ...
    describe('Rapid Toggle Stability', () => {
        it('should handle rapid hotkey presses without desync', async () => {
            ...
            expect(finalState?.enabled).toBe(!startEnabled);
            E2ELogger.info('always-on-top-hotkey', `Rapid toggle: started ${startEnabled}, ended ${finalState?.enabled}`);
