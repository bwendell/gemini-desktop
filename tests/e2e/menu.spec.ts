...
type MenuBrowser = {
    ...
    $(selector: string): Promise<MenuElement>;
    $$(selector: string): Promise<MenuElement[]>;
    electron: {
...
const menuBrowser = browser as unknown as MenuBrowser;
...
describe('Menu', () => {
    describe('Menu Bar', () => {
        ...
        beforeEach(async () => {
            ...
            const menuBar = await menuBrowser.$(Selectors.menuBar);
        ...
        it('should have menu buttons', async () => {
            ...
            const menuButtons = await menuBrowser.$$('.titlebar-menu-button');
        ...
        it('should have File, View, and Help menus', async () => {
            ...
            const menuBar = await menuBrowser.$(mainWindow.menuBarSelector);
            await menuBar.waitForExist();

            const allButtons = await menuBrowser.$$('.titlebar-menu-button');
        ...
        it('should open dropdown when File menu is clicked', async () => {
            ...
            const dropdown = await menuBrowser.$(Selectors.menuDropdown);
        ...
        it('should close dropdown when clicking outside (on backdrop)', async () => {
            ...
            const backdrop = await menuBrowser.$('.titlebar-menu-backdrop');
        ...
        it('should switch menus when hovering another menu button while open', async () => {
            ...
            const viewButton = await menuBrowser.$(Selectors.menuButton('View'));

            await mainWindow.openMenu('File');
            const dropdown = await menuBrowser.$(Selectors.menuDropdown);
            await dropdown.waitForExist();

            const exitItem = await menuBrowser.$(Selectors.menuItem('Exit'));
            ...
            await waitForUIState(
                async () => {
                    try {
                        const reloadItem = await menuBrowser.$(Selectors.menuItem('Reload'));
    ...
    describe('Actions', () => {
        ...
        it('should reload the page when clicking View -> Reload', async function (this: Context) {
            ...
            await menuBrowser.execute(() => {
                (window as { __e2e_test_var?: string }).__e2e_test_var = 'loaded';
            });

            const valBefore = await menuBrowser.execute(() => (window as { __e2e_test_var?: string }).__e2e_test_var);
            ...
            const valAfter = await menuBrowser.execute(() => (window as { __e2e_test_var?: string }).__e2e_test_var);
    ...
    describe('Edit Menu User Flow', () => {
        ...
        async function selectAllInputValue(inputId: string): Promise<void> {
            await menuBrowser.execute((targetId: string) => {
        ...
        async function clickEditRole(role: string): Promise<void> {
            const result = await menuBrowser.electron.execute(
        ...
        beforeEach(async () => {
            ...
            await menuBrowser.execute((inputId: string) => {
        ...
        it('should support copy, paste, cut, and undo through Edit menu roles', async () => {
            const input = await menuBrowser.$(`#${testInputId}`);
            await input.click();
            ...
            const selectionReady = await waitForUIState(
                async () => {
                    return menuBrowser.execute((inputId: string) => {
            ...
            const copyApplied = await waitForUIState(
                async () => {
                    const clipboardText = await menuBrowser.electron.execute((electron: typeof import('electron')) => {
    ...
    describe('Context Menu', () => {
        ...
        let testInput: MenuElement;

        beforeEach(async () => {
            await contextMenu.waitForAppReady();
            testInput = (await contextMenu.createTestInput()) as unknown as MenuElement;
